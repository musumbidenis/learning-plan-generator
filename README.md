...
[OS file] + [Curriculum file]
   ...
   │
   ├─(C) ONE GEMINI CALL        ─ ai_client.py                          [the only AI]
   │      └ fills learning_outcomes / activities / resources / assessments,
   │        grounded in parsed data; responseSchema forces valid JSON
   │
...
The API key is sent via `?key=` (not Bearer). Default model `gemini-2.5-flash`
with fallback `gemini-2.5-flash-lite`; HTTP 429 is retried, 401/403 surfaces a
clear "key invalid/expired" message.
...
| `ai_client.py` | C | the single grounded Gemini call + coercion/re-stamp/backfill safety nets |
...
- **Single AI call** uses `responseSchema` + `maxOutputTokens 32000` so long plans
